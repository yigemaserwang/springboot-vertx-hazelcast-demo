...
public class HttpVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) throws Exception {
		...
		System.out.println("HttpVerticle|start port=" + config().getInteger("port"));
		vertx.createHttpServer(options).requestHandler(request -> {
			String name = request.getParam("name");
			System.out.println("HttpVerticle|name=" + name);
			if (name == null) {
				request.response().setStatusCode(400).end("Missing name");
			} else {
				vertx.eventBus().<String>send("hello", name, ar -> {
					if (ar.succeeded()) {
						System.out.println("HttpVerticle|ar.succeeded()=" + name);
						request.response().end(ar.result().body());
					} else {
						System.out.println("HttpVerticle|else=" + name);
